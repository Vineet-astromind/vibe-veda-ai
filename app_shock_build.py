...
# Try to get key from Streamlit Secrets, else ask user
try:
    ...
    if st.button("🚀 Decode Aesthetic & Context"):
        ...
        if occasion == "Select Context...":
            ...
            with st.spinner(f"🧠 Orchestrating Agents for '{occasion}'..."):
                ...
                # --- 7. THE "SHOCK & BUILD" PROMPT (UPDATED) ---
                prompt = f"""
                ...
                Example: If the image is a plain shirt but context is 'Party', suggest styling it with bold chains and unbuttoning it.
                ...
                **Budget Pick:** [A cheaper alternative relevant to Indian students]
                ...
                try:
                    ...
                    # Spotify Search Link
                    spotify_query = urllib.parse.quote(f"{occasion} hindi vibes")
                    with col1:
                        st.link_button("🎵 Open Spotify", f"https://open.spotify.com/search/{spotify_query}")

                    # Myntra Search Link
                    shop_query = urllib.parse.quote(f"{occasion} outfit trendy")
                    with col2:
                        st.link_button("🛍️ Shop Myntra", f"https://www.myntra.com/{shop_query}")
